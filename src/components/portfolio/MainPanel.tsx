...
export function MainPanel({ activeConfig, booted, children, loadingModule, site, transitioning }: MainPanelProps) {
  return (
    <main
    ...
    >
      <CommandLine command={activeConfig.command} site={site} />
        ...
        {loadingModule && (
          <div className="absolute inset-0 flex items-start justify-center pt-20 bg-[#050812]/82 backdrop-blur-sm z-40">
            <div className="font-mono text-sm text-[#00ff88] border border-[#00ff88]/25 bg-[#050812] px-5 py-4 shadow-[0_0_24px_rgba(0,255,136,0.12)]">
              {site.loadingPrefix} {activeConfig.module}... <span className="animate-pulse">{site.loadingCompleteLabel}</span>
            </div>
          </div>
        )}
